...
const AdminProjects = (props) => {
  ...
  useEffect(() => {
    try {
      fetch('http://unn-w20017219.newnumyspace.co.uk/ic3/projects')
  ...
  const handleSelects = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/updateprojectext", {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const handleAddProject = async () => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/insertprojecttext", {
        method: 'POST',
        body: formData,
        headers: {
          Authorization: `Bearer ${token}`
  ...
  const onDeleteButtonClick = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/deleteprojecttext", {
        method: 'POST',
        body: formData,
        headers: {
          Authorization: `Bearer ${token}`
  ...
  function handleSubmit(event) {
    event.preventDefault();
    fetch('http://unn-w20017219.newnumyspace.co.uk/ic3/insertproject', {
      method: 'POST',
  ...
  return (
    <ParentPage>
      ...
      <label>
        Abstract:
        <input type="text" name="Abstract" value={abstract} onChange={event => setAbstract(event.target.value)} required />
      </label>
      <label>
        Main Text:
      ...
      <label>
        Partners:
        <input type="text" name="Partners" value={partners} onChange={event => setPartners(event.target.value)} required/>
      </label>
      <label>
        Image URL:
  ...
  <option value="not_completed">Not Complete</option>
    ...
    {searchedProjects.map((project) => (
          ...
          >
            ...
            <Link to={`/updateproject/${project.id}`} style={{ textDecoration: 'none' }}>
