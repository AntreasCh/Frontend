...
function UpdateProject() {
  ...
  useEffect(() => {
    fetch(`http://unn-w20017219.newnumyspace.co.uk/ic3/projects?id=${ids}`)
      .then((response) => {
        if (!response.ok) {
  ...
  function handleSubmit(event) {
    ...
    fetch(`http://unn-w20017219.newnumyspace.co.uk/ic3/updateproject`, {
      method: 'POST',
      body: formData,
    })
      .then(response => {
  ...
  const ProjectDelete = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/deleteproject", {
        method: 'POST',
        body: formData,
        headers: {
  ...
  return (
    ...
    <div class="updateprojectrow">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <label>
                  Abstract:
                  <textarea type="text" name="abstract" value={abstract}
                    onChange={event => setAbstract(event.target.value)} required />
                    </label>
                    <label>
                    Main Text:
