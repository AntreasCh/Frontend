...
function UpdateInnovation() {
  ...
  useEffect(() => {
    fetch(`http://unn-w20017219.newnumyspace.co.uk/ic3/innovation_projects?id=${ids}`)
      .then((response) => {
        if (!response.ok) {
  ...
  function handleSubmit(event) {
    ...
    fetch(`http://unn-w20017219.newnumyspace.co.uk/ic3/updateinnovationproject`, {
      method: 'POST',
      body: formData,
    })
      .then(response => {
  ...
  const ProjectDelete = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/deleteinnovationproject", {
        method: 'POST',
        body: formData,
        headers: {
