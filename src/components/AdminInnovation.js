...
const AdminInnovation = (props) => {
  ...
  useEffect(() => {
    try {
      fetch('http://unn-w20017219.newnumyspace.co.uk/ic3/innovation_projects')
  ...
  const handleSelects = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/updateinnovationtext", {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const handleAddProject = async () => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/innovationprojecttext", {
        method: 'POST',
        body: formData,
        headers: {
          Authorization: `Bearer ${token}`
  ...
  const onDeleteButtonClick = async (event, projectId) => {
    ...
    try {
      const response = await fetch("http://unn-w20017219.newnumyspace.co.uk/ic3/deleteinnovationtext", {
        method: 'POST',
        body: formData,
        headers: {
          Authorization: `Bearer ${token}`
  ...
  function handleSubmit(event) {
    event.preventDefault();
    fetch('http://unn-w20017219.newnumyspace.co.uk/ic3/insertinnovationproject', {
      method: 'POST',
  ...
  return (
    <ParentPage>
      {!props.loading && props.text && !loading && (
        <div className='Project_Text'>
          ...
          <h2>Add New  Innovation Projects</h2>
    ...
    {searchedProjects.map((project) => (
          ...
          >
            ...
            <Link to={`/updateinnovationproject/${project.id}`} style={{ textDecoration: 'none' }}>
